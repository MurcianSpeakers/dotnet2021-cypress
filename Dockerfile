...
COPY --from=buildspa /app/nginx/nginx.conf /etc/nginx/conf.d
COPY --from=buildspa /app/scripts/config/generate-config.sh ./generate-config.sh
COPY --from=buildspa /app/build /usr/share/nginx/html
